...
function Page4_03() {
  ...
  const fetchNewTaipeiYouBikeAPI = async () => {
    try {
      const response = await axios.get(
  ...
  console.log("render....");
  const {
    data: newTaipeiYoubike,
    status: newStatus,
    isLoading: newLoading,
    isFetching: newFetching,
  } = useQuery(["fetchNewTaipeiYouBikeAPI"], fetchNewTaipeiYouBikeAPI, {
    refetchOnWindowFocus: false,
    keepPreviousData: true,
    cacheTime: 60000,
    staleTime: Infinity, // 設置 staleTime 為 Infinity，表示即使過期，仍然使用舊資料直到新資料到來
  ...
  useEffect(() => {
    if (status === "success" || newStatus === "success") {
      ...
      console.log("newTaipeiYoubike", newTaipeiYoubike);
      setMarkers(data);
      setNewTaipeiMarkers(newTaipeiYoubike);
    }
  }, [data, newTaipeiYoubike]);
                ...
                >
                  ...
                  <Popup offset={[0, -13]}>
                    ...
                    >
                      ...
                      />
                      {isLoading || isFetching || newFetching || newLoading ? (
                        <>
                          <Spin
                            style={{
                              display: "flex",
