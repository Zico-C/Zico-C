...
function Page8_01() {
  ...
  useEffect(() => {
    // 使用過濾邏輯來找到匹配的數據
    const filtered = post.filter((item: any) => item.parameter[2].parameterValue.toLowerCase() === searchLocation);
    setFilteredData(filtered); // 更新filteredData狀態變數

    // 過濾匹配的地區名稱並存儲在suggestions中
    const matchingSuggestions = post
      .filter((item: any) => item.parameter[2].parameterValue.toLowerCase().includes(searchLocation.toLowerCase()))
