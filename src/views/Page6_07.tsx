...
function Page6_07() {
  ...
  const UserUp = User.map((user)=>{
    return user.name === "may" ?  { ...user , name:"XXX"} :
           user.name === "hank"? { ...user , age:30} :
           user;
  });
  ...
  return (
    <div className={styles.main}>
      <h1>Page6_07頁面 測試 useCallback</h1><br/>
      <p style={{
        color: (size.width > 1200) ? "green" : (size.width > 800) ? "blue" : "red"
      }}>{size.width} * {size.height}</p>
      <p>
